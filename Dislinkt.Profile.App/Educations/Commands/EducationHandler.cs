    ...
    class EducationHandler : IRequestHandler<EducationCommand, bool>
    {
        ...
        public async Task<bool> Handle(EducationCommand request, CancellationToken cancellationToken)
        {
            ...
            await _userRepository.AddEducationAsync(new User(existingUser.Id, existingUser.FirstName, existingUser.LastName, existingUser.Username, existingUser.Biography,
                ...
                existingUser.Status, updatedEducations.ToArray(), existingUser.WorkExperiences, existingUser.Skills, existingUser.Interests,existingUser.Seniority,existingUser.Role));
